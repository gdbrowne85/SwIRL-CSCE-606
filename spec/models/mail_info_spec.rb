...
RSpec.describe EventsController, type: :controller do
  describe 'GET #invite_attendees' do
    it 'sends email invitations to all attendees' do
      event = Event.create(name: 'Sample Event', id: 1)
      ...
      expect(EventRemainderMailer).to receive(:invite_email).exactly(2).times.and_return(double(deliver: true))
    ...
    it 'sends email invitations to a limited number of attendees when at max capacity' do
      ...
      expect(EventRemainderMailer).to receive(:invite_email).exactly(5).times.and_return(double(deliver: true))
